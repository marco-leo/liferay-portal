/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

import {CLOSE_MODAL} from 'commerce-frontend-js/utilities/eventsDefinitions';
import {createPortletURL} from 'frontend-js-web';

import AJAX from '../utils/AJAX/index';

export default function ({editCommerceDefaultSettingPortletURL, namespace}) {
	const form = document.getElementById(`${namespace}fm`);

	form.addEventListener('submit', (event) => {
		event.preventDefault();

		const name = form.querySelector(`#${namespace}name`).value;

		const commerceDefaultSettingData = {
			name,
		};

		return AJAX.POST(
			`/o/headless-commerce-qualifier/v1.0/default-settings`,
			commerceDefaultSettingData
		).then((payload) => {
			const redirectURL = createPortletURL(
				editCommerceDefaultSettingPortletURL
			);

			redirectURL.searchParams.append(
				`${namespace}commerceDefaultSettingId`,
				payload.id
			);
			redirectURL.searchParams.append('p_auth', Liferay.authToken);

			window.parent.Liferay.fire(CLOSE_MODAL, {
				redirectURL: redirectURL.toString(),
				successNotification: {
					message: Liferay.Language.get(
						'your-request-completed-successfully'
					),
					showSuccessNotification: true,
				},
			});
		});
	});
}
